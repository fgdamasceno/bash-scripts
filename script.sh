...
for nome in "${ven[@]}"
do
    useradd $nome -c "${nome} Silva" -s /bin/bash -m -p $(openssl passwd -6 ${nome}123) -G GRP_VEN
done
chmod 770 /ven
...
for nome in "${sec[@]}"
do
    useradd $nome -c "${nome} Silva" -s /bin/bash -m -p $(openssl passwd -6 ${nome}123) -G GRP_SEC
done
chmod 770 /sec
